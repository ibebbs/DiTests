...
namespace PureDiTests
{
    public partial class Tests
    {
        ...
        [Test]
        [Ignore("Func with dynamic values not supported")]
        public void GetDependencyWithParameterFactory()
        {
            Guid value = Guid.NewGuid();

            var dependencyFactory = Container.ServiceProvider.Get<IDependencyWithParameterFactory>();

            var dependency = dependencyFactory.Create(value);

            Assert.That(dependency, Is.Not.Null);
            Assert.That(dependency.Value, Is.EqualTo(value));
        }

        [Test]
        public void GetModuleDependency()
        {
            var dependency = Container.ServiceProvider.Get<IModuleDependency>();

            Assert.That(dependency, Is.Not.Null);
            Assert.That(dependency, Is.InstanceOf<ModuleDependency>());
        }
